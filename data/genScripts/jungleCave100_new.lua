...
MAP_CONFIGURATION = {
    ...
    logToFile = true,
    mainPos = {x = 1022, y = 1020, z = 7},
    mapSizeX = 100,
    mapSizeY = 100,
	mapSizeZ = 1, -- if set to greater than 1 => multi floor
    wpMinDist = 8,
    wayPointsCount = 32,
	mainGround = "jungle dirt",
	gravelType = "loose gravel",
	trash = true
}
...
GROUNDS = parseBrushes("grounds")
BORDERS = parseBrushes("borders")
DOODADS = parseBrushes("doodads")
local mainGround = GROUNDS[MAP_CONFIGURATION.mainGround]
local gravel = DOODADS[MAP_CONFIGURATION.mainGround]
print("GRAVEL", mainGround[1])
--local mainBorders = BORDERS[""]
...
function script.run()
	...
	--	print(dumpVar(wayPoints))
		...
		caveWayPointer:createPathBetweenWpsTSPMS(mainGround, 6, 3, currentFloor)

		local caveRoomBuilder = CaveRoomBuilder.new(generatedMap, wayPoints)
		caveRoomBuilder:createRooms(mainGround, 11, 11, currentFloor)
		...
		--generatedMap:correctGround(mainGround, ITEMS_TABLE[12][1], currentFloor)
		...
		groundRandomizer:randomizeByIds(mainGround, 40, currentFloor)
		groundRandomizer:randomizeByIds(mainGround, 70, currentFloor)
	...
	elevator:createGreyMountainRamps("random")
